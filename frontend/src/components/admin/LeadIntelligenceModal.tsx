...
export default function LeadIntelligenceModal({
  ...
  const [activeTab, setActiveTab] = useState<
    "AUDITORÍA" | "ESTRATEGIA" | "MERCADO" | "VENTAS"
  >("AUDITORÍA");
      ...
      <div className="relative bg-slate-50 w-full max-w-5xl h-[90vh] rounded-[40px] shadow-2xl border border-white/20 overflow-hidden flex flex-col">
        ...
        <div className="bg-white px-8 py-6 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 font-medium">
                Análisis de Mercado & Diagnóstico Digital
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {(lead.pipelineStatus === "NUEVO" ||
              lead.pipelineStatus === "CALIFICADO") && (
              <button
                onClick={async () => {
                  setIsPromoting(true);
                  try {
          ...
          <div className="flex gap-8">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-2 py-4 border-b-2 font-black text-xs uppercase tracking-widest transition-all ${
                  activeTab === tab.id
          ...
          {!intel ? (
            <div className="flex flex-col items-center justify-center h-full text-center space-y-6">
              ...
              <div>
                ...
                <p className="text-slate-500 max-w-sm mx-auto">
                  Click en &quot;Analizar&quot; para que nuestra IA diagnostique
                  este lead y genere un reporte estratégico.
                </p>
              </div>
            </div>
          ) : (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              {activeTab === "AUDITORÍA" && (
                <div className="space-y-8">
                  <section className="bg-white rounded-3xl p-6 border border-slate-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <h5 className="font-bold text-slate-900 border-b pb-2">
                          Technical Status
                        </h5>
                  ...
                  <IntelligenceMarkdown
                    content={
                      intel.strategicBrief ||
                      "No hay brief estratégico disponible."
                    }
                  />
                </div>
              )}
              ...
              {activeTab === "ESTRATEGIA" && (
                <div className="space-y-8">
                  <IntelligenceMarkdown
                    content={intel.strategicBrief || "Procesando estrategia..."}
                  />
                  <div className="bg-indigo-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden">
                    ...
                    <p className="text-indigo-100 leading-relaxed max-w-2xl">
                      {intel.strategicBrief?.split(".")[0] ||
                        "Generando perspectiva de negocio..."}
                      . Nuestro objetivo es capitalizar el gap digital para
                      aumentar su ROI.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white rounded-3xl p-6 border border-slate-100">
                      <h4 className="text-sm font-black uppercase text-slate-400 tracking-widest mb-4">
                        Market Analysis
                      </h4>
                      <IntelligenceMarkdown
                        content={
                          intel.marketAnalysis || "Cargando análisis local..."
                        }
                      />
                    </div>
                    <div className="bg-white rounded-3xl p-6 border border-slate-100">
                      ...
                      <IntelligenceMarkdown
                        content={intel.nicheAnalysis || "Evaluando nicho..."}
                      />
                    </div>
                  </div>
                </div>
              ...
              {activeTab === "VENTAS" &&
                (reviewingProposal ? (
                  <ProposalPreview
                    ...
                    proposal={reviewingProposal as any}
                  ...
                  <div className="space-y-8 animate-in fade-in duration-300">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white rounded-3xl p-6 border border-slate-100 space-y-4">
                        ...
                        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 relative group">
                          <p className="text-sm text-slate-700 italic">
                            {intel.whatsappMsg || "No generado"}
                          </p>
                          <button className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 text-[10px] font-black uppercase text-indigo-600 transition-all">
                            Copiar
                          </button>
                        </div>
                      </div>
                      <div className="bg-white rounded-3xl p-6 border border-slate-100 space-y-4">
                        ...
                        <div className="space-y-2">
                          <div className="text-xs font-bold text-slate-900 p-2 bg-slate-50 rounded-lg">
                            Asunto: {intel.emailSubject}
                          </div>
                          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-sm italic">
                            {intel.emailBody}
                          </div>
                        </div>
                      </div>
                      ...
                      {lead.proposal ? (
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                          <div className="space-y-2">
                            ...
                            <p className="text-indigo-200 text-sm max-w-lg">
                              Ya existe una propuesta para este lead. Puedes
                              verla online o enviarla de nuevo.
                            </p>
                          </div>
                              ...
                              onClick={async () => {
                                try {
                                  const res = await fetch(
                                    `/api/proposals/${lead.proposal.id}/send`,
                                    {
                                      method: "POST",
                                      headers: {
                        ...
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                          <div className="space-y-2">
                            ...
                            <p className="text-indigo-200 text-sm max-w-lg">
                              Utilizaremos toda la inteligencia recolectada más
                              tus notas de la llamada para redactar una
                              propuesta comercial premium personalizada.
                            </p>
                          </div>
                          <div className="flex-1 w-full max-w-md">
                            ...
                            <button
                              onClick={async () => {
                                if (!callNotes.trim()) {
                                  alert(
                                    "Por favor, ingresa notas de la llamada primero.",
                                  );
                                  return;
                                }
                    ...
                    <div className="bg-amber-50 rounded-3xl p-6 border border-amber-100">
                      ...
                      <IntelligenceMarkdown
                        content={
                          intel.interviewGuide ||
                          "Generando preguntas de clousure..."
                        }
                      />
                    </div>
                  </div>
        ...
        <div className="bg-white px-8 py-4 border-t border-slate-200 flex items-center justify-between">
          ...
          <div className="text-[10px] font-bold text-slate-400">
            Último análisis:{" "}
            {intel?.analyzedAt
              ? new Date(intel.analyzedAt).toLocaleString()
              : "Nunca"}
          </div>
        </div>
      </div>
