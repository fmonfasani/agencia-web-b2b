...
export default async function RevenueDashboard({
  ...
  type DealStage = { value: number | null; stage?: string };
  type ClosedWonDeal = {
    value: number | null;
    createdAt: Date;
    closedAt: Date;
  };
  type RevenueHistoryItem = { date: Date; revenueGenerated: number | null };
  ...
  const totalMrr = customerData.reduce(
    (sum: number, c: CustomerRevenue) => sum + Number(c.mrr || 0),
    0,
  );
  const newRevenueThisMonth = closedWonDeals.reduce(
    (sum: number, d: DealRevenue) => sum + Number(d.value || 0),
    0,
  );
  const pipelineValue = totalDeals.reduce(
    (sum: number, d: DealRevenue) => sum + Number(d.value || 0),
    0,
  );
  const conversionRate =
    totalDeals.length > 0
      ? (closedWonDeals.length / totalDeals.length) * 100
      : 0;
          ...
          <div className="pt-6 border-t border-slate-50">
            <div className="bg-slate-50 p-6 rounded-3xl space-y-3">
              ...
              <p className="text-xs text-slate-600 leading-relaxed font-medium">
                Tus cierres han aumentado un **12%** comparado al mes pasado. El
                ticket promedio subió, pero el ciclo de venta se extendió 3
                días.
              </p>
            </div>
          </div>
      ...
      <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {[
            ...
            const dealsInStage = totalDeals.filter((d) => d.stage === stage);
