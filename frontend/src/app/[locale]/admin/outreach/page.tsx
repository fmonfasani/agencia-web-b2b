...
const OutreachDashboard = async ({
  ...
  // Outreach campaigns are managed via backend API (no Prisma direct access).
  // Until the outreach API endpoint is implemented in backend-saas,
  // we render the UI with empty data rather than crashing.
  const campaigns: any[] = [];
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">
              Campaigns <span className="text-blue-600">Outreach</span>
            </h1>
            <p className="text-slate-500 font-medium max-w-xl">
              Gestiona y automatiza tus secuencias de contacto multi-canal para
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          <StatCard
            ...
            icon={Target}
            color="blue"
            description="Leads inscritos globalmente"
          />
          <StatCard
            title="Entregados"
            value={sentMessages}
            icon={Send}
            color="emerald"
            description="Mensajes salientes exitosos"
          />
          <StatCard
            ...
            icon={BarChart3}
            color="amber"
            description="Aperturas y clicks trackeados"
          />
          <StatCard
            title="Errores"
            ...
            icon={AlertCircle}
            color="rose"
            description="Problemas de entrega o API"
          />
        </div>
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <span className="font-black text-slate-900 text-sm group-hover:text-blue-600 transition-colors uppercase tracking-tight">
                          {c.name}
                        </span>
                    ...
                    <td colSpan={6} className="px-8 py-32 text-center">
                      <div className="flex flex-col items-center gap-4 text-slate-400">
                        <div className="p-6 bg-slate-50 rounded-full">
                          <Target
                            size={48}
                            className="opacity-20 translate-x-1 translate-y-1"
                          />
                        </div>
                        <div className="space-y-1">
                          ...
                          <p className="text-xs font-medium">
                            Inicia una nueva secuencia para comenzar a medir el
                            ROI.
                          </p>
                        </div>
                      </div>
...
const StatCard = ({
  ...
  return (
    <div className="bg-white p-7 rounded-[2.5rem] border border-slate-200 shadow-sm group hover:shadow-md transition-all relative overflow-hidden">
      <div
        className={`w-12 h-12 rounded-2xl ${colorClasses[color]} flex items-center justify-center mb-5 group-hover:scale-110 transition-transform`}
      >
        <Icon size={24} />
      </div>
      <div>
        ...
        {description && (
          <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-tighter opacity-70">
            {description}
          </p>
        )}
      </div>
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={`px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-tighter border ${styles[status] || styles.DRAFT}`}
    >
      {status}
    </span>
  );
