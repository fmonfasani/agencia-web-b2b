  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        addToast(
          `Agente ${next === "active" ? "activado" : "desactivado"}`,
          "success",
        );
      }
    } catch {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            {
              label: "Queries/día",
              value: details.metrics.queriesPerDay.toLocaleString(),
              color: "blue",
            },
            {
              label: "Latencia prom.",
              value: `${details.metrics.avgLatency.toFixed(0)}ms`,
              color: "yellow",
            },
            {
              label: "Tasa de éxito",
              value: `${details.metrics.successRate.toFixed(1)}%`,
              color: "green",
            },
            {
              label: "Tasa de error",
        ...
        <div>
          ...
          <div className="mt-6">
            ...
            {activeTab === "Performance" && (
              <AgentPerformance metrics={metrics} agentId={agentId} />
            )}
            {activeTab === "Logs" && <AgentLogs logs={logs} />}
            {activeTab === "Configuración" && (
              <AgentConfiguration agent={details} agentId={agentId} />
            )}
