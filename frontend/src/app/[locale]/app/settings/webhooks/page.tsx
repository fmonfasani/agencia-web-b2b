...
import {
  ...
  Webhook as WebhookType, WebhookLog, WebhookEvent, ALL_WEBHOOK_EVENTS,
...
export default function WebhooksPage() {
  ...
  const [form, setForm] = useState({ name: "", url: "", events: [] as WebhookEvent[] });
  ...
  const handleTest = async (whId: string) => {
    ...
    if (res.success) {
      addToast(`Webhook testeado — ${res.latency}ms — ${res.response}`, "success");
    } else {
  ...
  const handleViewLogs = async (whId: string) => {
    if (expandedLogs === whId) { setExpandedLogs(null); return; }
    if (!logs[whId]) {
        ...
        <StaggerItem>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">Recibe notificaciones automáticas en tu sistema</p>
            </div>
            <button
              ...
              >
                <div className="p-5 flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-mono text-gray-500 mb-2 break-all">{wh.url}</p>
                    ...
                    <div className="flex gap-4 mt-2 text-xs text-gray-500">
                      ...
                      {wh.lastFiredAt && <span>Último: {new Date(wh.lastFiredAt).toLocaleDateString("es-AR")}</span>}
                    </div>
                  </div>
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => handleViewLogs(wh.id)}
                ...
                <AnimatePresence>
                  {expandedLogs === wh.id && logs[wh.id] && (
                    ...
                    >
                      <div className="p-4">
                        <p className="text-xs font-semibold text-gray-600 mb-3">Últimos disparos</p>
                        <div className="space-y-2">
                          {logs[wh.id].map((log) => (
                            <div key={log.id} className="flex items-center gap-3 text-xs">
                              ...
                              <span className="text-gray-400 ml-auto">{new Date(log.timestamp).toLocaleString("es-AR")}</span>
                            </div>
                          ))}
                        </div>
                      </div>
