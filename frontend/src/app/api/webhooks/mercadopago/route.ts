...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!webhookSecret) {
      logger.error(
        "[WEBHOOK] MERCADOPAGO_WEBHOOK_SECRET not configured — rejecting all webhook requests",
      );
    ...
    if (!isValid) {
      logger.warn("[WEBHOOK] Invalid MercadoPago signature", {
        xRequestId,
        dataId,
    ...
    logger.info("[WEBHOOK] MercadoPago webhook received", {
      type: type || body.type,
      action: body.action,
    ...
    if (eventType === "payment" && eventDataId) {
      ...
      logger.warn("[WEBHOOK] Unhandled webhook type", { eventType });
    }

    // 6. ALWAYS return 200 OK (MercadoPago requires this to stop retries)
    return NextResponse.json({ received: true });
  } catch (error) {
    logger.error("[WEBHOOK] MercadoPago webhook error", {
