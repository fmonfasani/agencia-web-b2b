...
// Configuración de Axios - En producción esto debería venir de una variable de entorno
const API_BASE_URL =
      ...
      try {
        const res = await api.get("/leads", {
          params: {
            ...
            has_socials:
              hasSocials === "all" ? undefined : hasSocials === "yes",
            has_whatsapp:
              hasWhatsapp === "all" ? undefined : hasWhatsapp === "yes",
            has_website:
              hasWebsite === "all" ? undefined : hasWebsite === "yes",
            limit: 100,
          },
        });
        setLeads(res.data.leads);
            ...
            {loadingDetail ? (
              ...
              <div className={styles.detailGrid}>
                <div className={styles.detailSection}>
                  ...
                  <div className={styles.detailItem}>
                    <strong>Sector:</strong> {selectedLead.sector}
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Categoría:</strong> {selectedLead.categoria}
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Email:</strong> {selectedLead.email || "N/A"}
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Web:</strong> {selectedLead.website || "N/A"}
                  </div>
                  {selectedLead.whatsapp_number && (
                    <div className={styles.detailItem}>
                      <strong>WhatsApp:</strong> +{selectedLead.whatsapp_number}
                    </div>
                  )}
                </div>
                <div className={styles.detailSection}>
                  ...
                  <div className={styles.detailItem}>
                    <strong>Digital Score:</strong>{" "}
                    {selectedLead.digital_score_lice}%
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Prioridad:</strong> {selectedLead.priority_score} (
                    {selectedLead.priority_tag})
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Confianza IA:</strong>{" "}
                    {Math.round((selectedLead.confidence || 0) * 100)}%
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Redes:</strong>{" "}
                    {selectedLead.redes_detectadas || "Ninguna"}
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Web Propia:</strong>{" "}
                    {selectedLead.tiene_web_propia ? "Sí" : "No"}
                  </div>
                  <div className={styles.detailItem}>
                    <strong>Fuente:</strong> {selectedLead.fuente}
                  </div>
                </div>
              </div>
            ) : (
          ...
          <div className={styles.modalFooter}>
            ...
            {selectedLead?.whatsapp_number && (
              <button
                ...
                onClick={() =>
                  window.open(`https://wa.me/${selectedLead.whatsapp_number}`)
                }
              >
                WhatsApp
              </button>
    ...
    <div className={styles.dashboardWrapper}>
      ...
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.sidebarNav}>
          <div
            className={`${styles.navItem} ${activeTab === "explorer" ? styles.navItemActive : ""}`}
          ...
          <div
            className={`${styles.navItem} ${activeTab === "top" ? styles.navItemActive : ""}`}
        ...
        <header className={styles.topBar}>
          ...
          <div className={styles.topBarActions}>
            <button
              ...
              onClick={() =>
                window.open(
                  `${API_BASE_URL}/classify/download/leads_prioritarios_alta.csv`,
                )
              }
            >
        ...
        <div className={styles.pageContent}>
          <div className={styles.pageHeader}>
            ...
            <p className={styles.pageSubtitle}>
              {activeTab === "explorer"
                ? "Consulta y segmentación del universo completo de leads."
                : "Ranking de los leads con mayor potencial comercial detectado por LICE."}
            </p>
          </div>
          ...
          <div className={styles.statsRow}>
            <div className={styles.statCard}>
              ...
              <div className={styles.statValue}>
                {summary?.total_leads?.toLocaleString() || "42,983"}
              </div>
            </div>
            <div className={styles.statCard}>
            ...
            <div className={styles.filtersBar}>
              ...
              {currentSector && (
                ...
                >
                  ...
                  {Object.keys(taxonomy[currentSector]).map((c) => (
              ...
              <select
                className={styles.filterSelect}
                value={hasSocials}
                onChange={(e) => setHasSocials(e.target.value)}
              >
                ...
                {loading
                  ...
                  : leads.map((lead) => (
                      <tr key={lead.id}>
                        ...
                        <td
                          className={`${styles.textBold} ${styles.truncatedCell}`}
                          title={lead.name}
                        >
                          {lead.name}
                        </td>
                        ...
                        <td>
                          ...
                          >
                            {lead.redes_detectadas ? (
                              lead.redes_detectadas.split(",").map((r) => (
                                <span
                                  key={r}
                                  style={{
                                    fontSize: "10px",
