...
export default async function SignInPage({
  ...
  const t = {
    ...
    apple: en ? "Sign in with Apple" : "Continuar con Apple",
    or: "or",
    badge: en ? "Protected by Revenue OS IAM · TLS 1.3" : "Protegido por Revenue OS IAM · TLS 1.3",
    tagline: en
      ? "Your sales pipeline, humans and AI,\nall in one place."
      : "Tu pipeline de ventas, humanos e IA,\ntodo en un solo lugar.",
    stats: en
      ? [{ label: "Active companies", value: "2,400+" }, { label: "MRR generated", value: "$1.2M" }, { label: "AI agents", value: "12K+" }]
      : [{ label: "Empresas activas", value: "2,400+" }, { label: "MRR generado", value: "$1.2M" }, { label: "Agentes IA", value: "12K+" }],
  };
    ...
    <div className="min-h-screen bg-[#0d0f14] flex">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6 py-12 overflow-y-auto">
        <div className="w-full max-w-[420px]">
          ...
          <h1 className="text-[2rem] font-black text-white leading-tight">{t.heading}</h1>
          <p className="text-[#8b92a5] mt-1.5 text-sm">
            {t.sub}{" "}
            <Link href={`/${locale}/auth/register-company`} className="text-[#135bec] font-semibold hover:text-blue-400 transition-colors">
              {t.signUp}
            </Link>
          </p>
          ...
          <div className="mt-8 space-y-4">
            {/* Google */}
            <button type="button" className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl border border-[#2a2f3e] bg-[#161923] text-white text-sm font-medium hover:bg-[#1e2435] hover:border-[#3a4055] transition-all">
      ...
      <div className="hidden lg:flex w-1/2 relative flex-col items-center justify-center overflow-hidden bg-[#0a0d13]">
        ...
        <div className="absolute inset-0 z-0">
          <Image src="/signin-hero.png" alt="Revenue OS AI Agent" fill className="object-cover object-center" priority />
        ...
        <div className="relative z-10 mt-auto mb-40 text-center px-12">
          <p className="text-[#8b92a5] text-sm leading-relaxed whitespace-pre-line">{t.tagline}</p>
          <div className="mt-5 flex items-center justify-center gap-2">
            {[...Array(3)].map((_, i) => (
              <div key={i} className={`rounded-full ${i === 0 ? "w-6 h-1.5 bg-[#135bec]" : "w-1.5 h-1.5 bg-[#2a2f3e]"}`} />
            ))}
          </div>
        </div>
