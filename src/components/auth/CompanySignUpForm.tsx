...
import { Lock, Mail, Building2, Phone, Globe, ShieldCheck, User, ChevronDown } from "lucide-react";
import Link from "next/link";

const PLANS = [
    { code: "STARTER", label: "Starter — $49/mes", description: "3 usuarios, 1 agente IA, 1 canal" },
    { code: "PRO", label: "Pro — $149/mes", description: "10 usuarios, 5 agentes IA, canales ilimitados" },
    { code: "ENTERPRISE", label: "Enterprise — $499/mes", description: "Ilimitado, API access, Webhooks" },
];
...
export default function CompanySignUpForm({ darkMode = false, locale = "es" }: CompanySignUpFormProps) {
    ...
    async function onSubmit(event: FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await fetch(`/${locale}/api/auth/register-company`, {
                ...
                body: JSON.stringify({ firstName, lastName, email, whatsapp, companyName, website: hasWebsite ? websiteUrl : null, plan, password }),
            });

            const data = await response.json();
            if (!response.ok) { setError(data.error || "Error al completar el registro"); setLoading(false); return; }
    ...
    const field = "w-full rounded-xl border border-[#2a2f3e] bg-[#161923] pl-10 pr-4 py-3 text-sm text-white placeholder-[#4a5168] outline-none focus:ring-2 focus:ring-[#135bec]/50 focus:border-[#135bec]/50 transition-all font-medium";
    const label = "text-xs font-semibold text-[#8b92a5] uppercase tracking-widest";
            ...
            <form onSubmit={onSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                    ...
                    <div className="relative">
                        <ChevronDown className="absolute right-3.5 top-1/2 -translate-y-1/2 text-[#4a5168]" size={15} />
                ...
                <label className="flex items-center gap-2.5 cursor-pointer group">
                    <div className="relative" onClick={() => setHasWebsite(!hasWebsite)}>
                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${hasWebsite ? "bg-[#135bec] border-[#135bec]" : "border-[#2a2f3e] bg-[#161923]"}`}>
                ...
                <p className="text-center text-xs text-[#4a5168]">
                    ¿Ya tenés cuenta?{" "}
                    <Link href={`/${locale}/auth/sign-in`} className="text-[#135bec] font-semibold hover:text-blue-400 transition-colors">
                        Iniciá sesión →
                    </Link>
                </p>
            </form>
    ...
    return (
        <form onSubmit={onSubmit} className="space-y-5">
            <div className="grid grid-cols-2 gap-3">
                <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} required className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="Nombre" />
                <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} required className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="Apellido" />
            </div>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="Email" />
            <input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} required className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="Empresa" />
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="Contraseña" />
            <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm" placeholder="Confirmar contraseña" />
            {error && <p className="text-red-600 text-sm">{error}</p>}
            <button type="submit" disabled={loading} className="w-full rounded-xl bg-primary text-white py-3 font-semibold text-sm disabled:opacity-50">
