...
function TruncateText({ text, maxLength = 40 }: { text?: string | null; maxLength?: number }) {
    ...
    return <span title={text} className="cursor-help">{text.substring(0, maxLength)}…</span>;
}

export default function LeadsDataTable({ leads: initialLeads, tenantId }: { leads: any[], tenantId?: string }) {
    ...
    const {
        ...
        sortField, sortDir,
        page, setPage,
        selectedLeadIds, toggleLeadSelection, toggleAllOnPage, toggleSort,
        selectedLead, setSelectedLead,
        isAnalyzing, paginated, totalPages,
        handleAnalyze, exportCSV
    } = useLeadsTable({ leads });

    const [isEnrollModalOpen, setIsEnrollModalOpen] = useState(false);

    const Th = ({ children, field, className = "" }: any) => (
        <th
                    ...
                    <div className="flex items-center p-1 bg-slate-50 rounded-2xl">
                        {TABS_CONFIG.map((t) => (
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${activeTab === t.id ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:text-slate-600"
                                    }`}
                            >
                                {t.label}
                            </button>
                    ...
                    <thead>
                        <tr>
                            <th className="w-10 px-3 py-3 border-b border-slate-100 bg-slate-50/50">
                                <button onClick={toggleAllOnPage} className="text-slate-300 hover:text-indigo-600">
                                    <Layers size={18} />
                                </button>
                            </th>
                            <Th className="w-8">#</Th>
                            ...
                            return (
                                ...
                                >
                                    <td className="px-3 py-3 border-b border-slate-100 group-selection">
                                        ...
                                        >
                                            {selectedLeadIds.includes(lead.id) ? <CheckSquare className="text-indigo-600" /> : <Square />}
                                        </button>
                                    </td>
                                    <Cell className="text-slate-300 tabular-nums">{(page - 1) * 50 + idx + 1}</Cell>
                                    <Cell className="min-w-[200px]">
                                        <div className="font-bold text-slate-800">{lead.name}</div>
                                        <div className="text-[10px] text-slate-400 uppercase font-black">{lead.category || lead.description || 'Sin categoría'}</div>
                                    </Cell>
                                    ...
                                    {activeTab === "overview" && (
                                        <>
                                            ...
                                            <Cell className="tabular-nums font-medium">{lead.reviewsCount || 0}</Cell>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <div className="h-1 w-12 bg-slate-100 rounded-full overflow-hidden">
                                                        <div
                                                            ...
                                                            style={{ width: `${lead.intelligence?.opportunityScore ?? lead.potentialScore}%` }}
                                                        />
                                                    </div>
                                                    <span className="text-xs font-bold text-slate-500">{lead.intelligence?.opportunityScore ?? lead.potentialScore}</span>
                                                </div>
                                            </Cell>
                                        </>
                                    )}
                                    {activeTab === "contact" && (
                                        <>
                                            <Cell>
                                                {lead.phone ? (
                                                    <a href={`tel:${lead.phone}`} className="text-indigo-600 hover:underline font-medium">{lead.phone}</a>
                                                ) : '—'}
                                            </Cell>
                                            <Cell className="max-w-[180px]">
                                                {lead.website ? (
                                                    <a href={lead.website} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline truncate block">
                                            ...
                                            <Cell>
                                                {lead.googleMapsUrl ? (
                                                    <a href={lead.googleMapsUrl} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline flex items-center gap-1">
                                    ...
                                    {activeTab === "social" && (() => {
                                        ...
                                        return (
                                            <>
                                                <Cell><SocialLink url={ig} type="instagram" /></Cell>
                                                <Cell><SocialLink url={fb} type="facebook" /></Cell>
                                                <Cell><SocialLink url={wa} type="whatsapp" /></Cell>
                                                <Cell><SocialLink url={lead.linkedin} type="linkedin" /></Cell>
                                            </>
                                        );
                                    })()}
                                    {activeTab === "location" && (
                                        <>
                                            <Cell><TruncateText text={lead.address} /></Cell>
                                            <Cell>{raw.city || '—'}</Cell>
                                            <Cell className="font-mono text-[10px]">{raw.location?.lat?.toFixed(4) || '—'}</Cell>
                                            <Cell className="font-mono text-[10px]">{raw.location?.lng?.toFixed(4) || '—'}</Cell>
                                        </>
                                    )}
                                    {activeTab === "rating" && (
                                        <>
                                            <Cell>{lead.rating ? <StarRating score={lead.rating} /> : '—'}</Cell>
                                            <Cell className="tabular-nums font-bold">{lead.reviewsCount || 0}</Cell>
                                            <Cell className="text-[10px] font-black uppercase text-slate-400">{lead.category || 'N/A'}</Cell>
                                        </>
                                    )}
                                    {activeTab === "all" && (
                                        <>
                                            <Cell>{lead.phone || '—'}</Cell>
                                            <Cell><TruncateText text={lead.website} maxLength={20} /></Cell>
                                            <Cell><TruncateText text={lead.address} maxLength={20} /></Cell>
                                            <Cell><StatusBadge status={lead.status} /></Cell>
                                        </>
                                    )}
                                </tr>
